...
export const uploadPhoto = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { error: insErr, data: photoId } = await sb.rpc("submit_guest_photo", {
      ...
      p_storage_url: path,
      p_filter_applied: data.filter ?? null,
      p_original_url: originalPath ?? null,
...
export const getMyHost = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase.from("hosts")
      .select("user_id, email, status, full_name, phone, company, event_interest, contact_submitted, contact_updated_at, created_at")
