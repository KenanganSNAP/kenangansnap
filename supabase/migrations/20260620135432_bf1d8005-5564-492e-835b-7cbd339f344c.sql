CREATE OR REPLACE FUNCTION public.register_guest(p_slug text, p_name text, p_token text DEFAULT NULL)
...
BEGIN
  ...
  -- Idempotent: if a guest with this token already exists, return it.
  ...
  v_token := COALESCE(NULLIF(p_token, ''), encode(gen_random_bytes(32), 'hex'));
